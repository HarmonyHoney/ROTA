...
@export var is_editor := false: set = set_is_editor
@export var day_clock := 0.0
@export var day_time := 420.0: set = set_day_time
@export var day_scale := 1.0: set = set_day_scale
@export var sky_pal = [Color("ffa300"), Color("00e0ff"), Color("0062ff"), Color("ac00ff"), Color("af00bf"), Color("250000")]: set = set_sky_pal
...
@onready var particles := $Hide/GPUParticles2D
...
@onready var star_light := $Sky/Center/Stars/Orbit/PointLight2D
...
@export var snow_mat : ParticleProcessMaterial
@export var snow_tex : Texture2D
@export var rain_mat : ParticleProcessMaterial
@export var rain_tex : Texture2D

var sun_frac := 0.5
var moon_frac := 0.5

@export var is_rain := false: set = set_is_rain
var is_snow := false
var rain_clock := 60.0
...
func _enter_tree():
	...
	Cam.connect("moved", Callable(self, "cam_moved"))

func _ready():
	star_light.enabled = !Engine.is_editor_hint()
	...
	moon_mat.set_shader_parameter("time_add", lerp(0.1, 0.9, randf()))
...
func _process(delta):
	...
	if sky_mat and moon_mat:
		for i in 2:
			for m in [sky_mat, moon_mat]:
				m.set_shader_parameter("col" + str(i + 1), sky_pal[sky_step - 2 - i].lerp(sky_pal[sky_step - 1 - i], step_frac))
	...
	moon_mat.set_shader_parameter("sun_frac", sun_frac)
	
	starfield.modulate.a = moon_frac
	starfield.visible = moon_frac > 0
	
	var mf = MenuMakeover.fade_ease
	canvas_mod.color = color_bright.lerp(color_dark, max(moon_frac - mf.smooth(), 0.0))
	if !mf.is_last:
	...
	cloud_rotation = fposmod(cloud_rotation + deg_to_rad(cloud_speed * delta * cloud_dir * day_scale), TAU)
...
func _physics_process(delta):
	if Engine.is_editor_hint() or Shared.is_arcade: return
...
func set_is_editor(arg := is_editor):
	...
	if !is_editor:
		day_clock = 0.0
		if sky_mat:
			sky_mat.set_shader_parameter("col1", sky_pal[0])
			sky_mat.set_shader_parameter("col2", sky_pal[1])
...
func set_is_rain(arg := is_rain):
	...
	rain_clock = randf_range(vec.x, vec.y)
...
func create_clouds():
	cloud_dir = (-1.0 if randf() > 0.5 else 1.0) * randf_range(0.6, 1.0)
	precip_list = []
	var pi = 0
	...
	for x in 0 if Shared.is_arcade else( (length / 50.0) + cloud_bonus_rings):
		for y in max(3, x):
			...
			var scl = Vector2.ONE * randf_range(0.25, 2.0)
			var dist = ((x + 2) * 200) + randf_range(0.0, 100.0)
