...
var positions := []
var cursor_margin := Vector2(-50, 0)
...
func _ready():
	...
	var h = $Control/List/Items/Up/HBoxContainer
	
	start = h.rect_global_position
	
	for i in h.get_children():
		positions.append(i.rect_position)
	
	cursor_node.rect_size = h.get_child(0).rect_size
...
func _physics_process(delta):
	...
	var c = cursor_node.rect_position
	var l = list_node.rect_position
	var i = items[cursor - scroll].rect_position
	
	c.x = lerp(c.x, start.x + positions[cursor_x].x , 0.15)
	c.y = lerp(c.y, l.y + i.y, 0.15)
	
	cursor_node.rect_position = c
