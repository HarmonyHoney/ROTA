...
func _ready():
	...
	Steam.set_leaderboard_score("High Score", 982, Steam.LeaderboardUploadScoreMethod.KeepBest, PackedInt32Array([5, 5, 3]))

	for i in Steam.user_stats.get_num_achievements():
	   print(Steam.user_stats.get_achievement_name(i))

	var scores = await Steam.get_leaderboard_scores("High Score", 0, 10, Steam.LeaderboardDataRequest.Global, 3).done
	print(scores)

	Steam.friends.connect("game_overlay_activated", Callable(self, "_on_game_overlay_activated"))
